...
class ProjectController extends Controller
{
    public function index(Request $request)
    {
       ...
       (p.overall_price -
        case when sum(i.price) is null then 0 else sum(i.price) end)                                           as ostalos,
       p.year_profit                                                                                           as godovoy_dohod_za_odin_procent,
       (p.overall_price - case when sum(i.price) is null then 0 else sum(i.price) end) * 100 /
       p.overall_price                                                                                         as ostalos_procentov,
...
group by p.id, p.title, p.overall_price, p.year_profit, p.end_date
having ostalos > 0
   and god > 0;";
            $data = DB::select($sql);
